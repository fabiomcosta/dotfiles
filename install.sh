...
install() {
    ...
    if [ ! `which $command` ]; then
        ...
        $@
        if [[ "$?" ]]; then
            echo "${ERROR} a problem happened while installing $command."
            exit 1
        else
            echo "${OK} $command successfully installed."
        fi
    else
        echo "${OK} $command is already installed."
    fi
}
...
if [ ! -e $HOME/.vim/bundle/vundle ]; then
    ...
    echo "${OK} vundle is already installed."
fi
