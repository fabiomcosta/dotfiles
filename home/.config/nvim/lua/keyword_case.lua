-- \k is a special vim regex character that matches a keyword, considering
-- the iskeyword option.
-- So this function will return if a character is a keyword as defined by the
-- current buffer iskeyword option.
local function match_keyword(_char)
  return vim.fn.match(_char, '\\k') ~= -1
end
...
local function get_non_range_keywords()
  ...
  for _, value in pairs(vim.opt.iskeyword:get()) do
    ...
    elseif value ~= '@' and #value == 1 then
      ...
      keywords = keywords .. regex_escape(value)
    end
  end
  return keywords
end
...
local function is_snake_case(word)
  local keywords = get_non_range_keywords()
...
local function replace_word_under_cursor(transformer)
  ...
  while
    start_col > 0 and match_keyword(line:sub(start_col - 1, start_col - 1))
  do
    start_col = start_col - 1
  end

  while end_col < #line and match_keyword(line:sub(end_col + 1, end_col + 1)) do
    end_col = end_col + 1
  end
